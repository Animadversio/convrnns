...
expdir = join(savedir, f"{args.model_name}-{layername}_dyn_BigGAN")
    ...
    for time_idx in range(len(y[layername])):  # todo this is the shape of the output
        for rep in range(reps):
            ...
            for methodlab, optimizer in zip(method_col, optimizer_col):
                ...
                plt.imsave(join(expdir, f"{layername}_chan{chan_id:03d}_T{time_wdw_str}_lastgen_{methodlab}_{RND}.png"),
                           imgs.permute(0, 2, 3, 1).numpy()[0], )
                np.savez(join(expdir, f"scores_{layername}_chan{chan_id:03d}_T{time_wdw_str}_{methodlab}_{RND}.npz"),
                         **savedict)
                mtg_exp = ToPILImage()(make_grid(best_imgs, nrow=10))
                mtg_exp.save(join(expdir, f"besteachgen_{layername}_chan{chan_id:03d}_T{time_wdw_str}_{methodlab}_{RND}.jpg" ))
                mtg = ToPILImage()(make_grid(imgs, nrow=7))
                mtg.save(join(expdir, f"lastgen_{layername}_chan{chan_id:03d}_T{time_wdw_str}_{methodlab}_{RND}.jpg"))
