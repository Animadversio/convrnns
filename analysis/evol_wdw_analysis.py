...
figdir = r"F:\insilico_exps\convrnn_Evol\fasrc_runs\summary\BGFC_timewdw_psth_cmp"
...
for layernum in trange(1, 10+1):
    ...
    for channum in trange(50):
        ...
        plt.subplots(1, 1, figsize=(5.5, 4.5))
        ...
        plt.legend()
        plt.title(f"{model_name} Layer {layernum} Channel {channum},  Objective T{T_active}-T16")
        plt.xlabel("Time steps (10ms)")
        plt.ylabel("Activation")
        saveallforms(figdir, f"{model_name}_{layernum}_chan{channum}_T{T_active}-T16_obj_final_psth_cmp")
